...
exam %>% summarise(n = n()) # group_by와 주로 같이 씀. 써머라이즈만 단독으로 쓰거나 다른 함수와 같이써도됨
# 하지만 group_by 없이 쓰면 별로 효용이 없다.
# n()은 exam 전체 행에대하여라는 함수.
# summarise는 통계 뿐만아니라 평균같은것도 구할 수 있음.
exam %>% tally()
# 위에 두개 결과 같음
# 그루핑 안한 첫째 코드는 전체에 대하여 요약한다는거임.
# group_by 하면 그룹한거에 대해서 요약.
# tally는 요약된 summarise임..
...
# 4명이 있다. data.frame을 가지고 할 수 있는 모든것은 티불을 가지고 쓸 수있음.
# 티불은 그것을 업그레이드 시킨것임.
exam %>%
  group_by(class) %>% tally() # 개수 세는게 전부일때 tally

exam %>% count(class)         # count() is a short-hand for group_by() + tally()

# add_tally() 와 ass_count(..) 도 있음
# 새롭게 추가하는것.
# 변수를 기준으로 그루핑하여 카운트해줌.
?count
...
str(mpg)
# mpg를 사용하려면 ggplot2를 로드해야 함.
# 각 집단별로 다시 집단 나누기
# mpg는 1999 - 2008 자동차 유용한 정보~~ 
mpg %>%
...
# class 기준 합치기
exam_new <- left_join(exam, name, by = "class") # 티처 컬럼 추가됨..
#righ_join -> 6제외
#left_join -> 7제외
#full_join -> 6,7 포함.
# 세로로 합치기
